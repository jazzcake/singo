...
int ice_create_local_description_with_account(ice_description_t *description, const char* ice_uflag, const char* ice_pwd) {
	memset(description, 0, sizeof(*description));
	memcpy(description->ice_ufrag, ice_uflag, strlen(ice_uflag) + 1);
	memcpy(description->ice_pwd, ice_pwd, strlen(ice_pwd) + 1);
	description->candidates_count = 0;
	description->finished = false;
	JLOG_DEBUG("Created local description: ufrag=\"%s\", pwd=\"%s\"", description->ice_ufrag,
	           description->ice_pwd);
...
int ice_generate_candidate_sdp(const ice_candidate_t *candidate, char *buffer, size_t size) {
	JLOG_DEBUG("ice_generate_candidate_sdp");
