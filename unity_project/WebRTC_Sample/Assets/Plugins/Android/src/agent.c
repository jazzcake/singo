...
juice_agent_t *agent_create_with_account(const juice_config_t *config, const char* ice_uflag, const char* ice_pwd) {
	JLOG_VERBOSE("Creating agent");
	...
	ice_create_local_description_with_account(&agent->local, ice_uflag, ice_pwd);
...
int agent_gather_candidates(juice_agent_t *agent) {
	...
	JLOG_VERBOSE("Socket: %s, %d~%d\n", socket_config.bind_address, socket_config.port_begin, socket_config.port_end);
	...
	// 기존 agent에 지정된 값으로 addr_record_t를 셋팅한다.
	...
	// 구한 record 값으로 ice candidate를 구한 뒤 local.candidates에 추가한다.
